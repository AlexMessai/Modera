...
export async function GET() {
  ...
  return Response.json({ data: await getGlobalReportProfile() });
}
...
export async function PATCH(request: Request) {
  ...
  if (!canManageChatSettings(auth.admin.role)) {
    return Response.json({ error: { code: "FORBIDDEN", message: "Изменять политику жалоб могут только владелец и администратор Modera." } }, { status: 403 });
  }
  ...
  const saved = await updateGlobalReportProfile({
