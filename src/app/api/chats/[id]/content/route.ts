...
export async function GET(
  ...
  const profile = await getChatContentProfile(id);
  if (!profile) {
...
export async function PATCH(
  ...
  if (!canManageChatSettings(auth.admin.role)) {
    return Response.json({ error: { code: "FORBIDDEN", message: "Изменять приветствие/правила могут только владелец и администратор Modera." } }, { status: 403 });
  }
