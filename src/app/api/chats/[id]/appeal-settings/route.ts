...
import {
  getChatAppealProfile,
  updateChatAppealProfile
} from "@/server/services/chat-appeal-settings-service";

export const dynamic = "force-dynamic";

const settingsSchema = z.object({
  enabled: z.boolean(),
  notifyAdminsOnSubmit: z.boolean(),
  notifyUserOnDecision: z.boolean()
});
...
export async function GET(
  ...
  const profile = await getChatAppealProfile(id);
  if (!profile) {
...
export async function PATCH(
  ...
  if (!canManageChatSettings(effectiveRole)) {
    return Response.json({ error: { code: "FORBIDDEN", message: "Изменять настройки апелляций могут только владелец и администратор Modera." } }, { status: 403 });
  }
  const parsed = settingsSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return Response.json({ error: { code: "VALIDATION_ERROR", message: parsed.error.issues[0]?.message ?? "Проверьте настройки апелляций." } }, { status: 400 });
  }
