...
export async function PATCH(
  ...
  if (!canManageChatSettings(auth.admin.role)) {
    return Response.json({ error: { code: "FORBIDDEN", message: "Изменять Anti-Raid настройки могут только владелец и администратор Modera." } }, { status: 403 });
  }
  ...
  const { id } = await context.params;
  const saved = await updateChatAntiRaidProfile({
    chatId: id,
