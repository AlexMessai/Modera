...
type JournalItem = {
  ...
  chat: Chat | null;
  affectedUser: Person | null;
  actingAdmin: { id: string; displayName: string; email: string } | null;
};
...
type PendingItem = {
  ...
  chat: Chat;
  affectedUser: Person;
  actingAdmin: { id: string; displayName: string; email: string };
};
...
async function requestJournal(input: {
  page: number;
  ...
  if (!response.ok) {
    throw new Error(payload?.error?.message ?? "Не удалось загрузить журнал.");
  }
        ...
        <section className="panel pending-panel">
          <div className="panel-header">
            <div>
              ...
              <p>Telegram-действия, которые остались в PENDING и не должны потеряться.</p>
            </div>
            ...
            {data.pending.map((item) => (
              <div className="pending-row" key={item.id}>
                ...
                <div>
                  ...
                  <span>{item.chat.title} · {item.actingAdmin.displayName}{item.reason ? ` · ${item.reason}` : ""}</span>
                </div>
        ...
        <div className="toolbar toolbar--journal">
          <form className="search-box" onSubmit={applySearch}>
            ...
            <input
              ...
              placeholder="Пользователь, чат, администратор или причина"
          ...
          <div className="journal-filters">
            <select
              ...
              value={category}
              onChange={(event) => {
                setCategory(event.target.value as JournalCategory);
                setPage(1);
                setLoading(true);
              }}
                ...
                <tbody>
                  {data.items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>
                        {item.chat ? <Link className="table-link" href={`/chats/${item.chat.id}`}>{item.chat.title}</Link> : "—"}
                      </td>
                      <td>
                        {item.affectedUser ? (
                          <Link className="stacked-cell table-link" href={`/members/${item.affectedUser.id}`}>
                            <strong>{item.affectedUser.displayName}</strong>
                            <span>{item.affectedUser.username ? `@${item.affectedUser.username}` : item.affectedUser.telegramUserId}</span>
                          </Link>
                        ) : "—"}
                      </td>
